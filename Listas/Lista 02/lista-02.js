...
const informaDadosEstudante = () => {
  console.log(`A estudante ${estudante.nome} tem ${estudante.idade}, cursa ${estudante.curso} e tiras as seguintes notas no 1º semestre de 2023:`)
...
const encontraCapital = (nomePais, paises) => paises.hasOwnProperty(nomePais) ? `A capital de ${nomePais} é ${paises[nomePais]}` : `Capital não encontrada`
...
const retornaPrimeiraLetra = palavras => palavras.map((palavra) => palavra[0].toUpperCase())
...
const calculaValorTotal = produtosNerd => produtosNerd.map((produto) => `O valor total do produto ${produto.nome} é R$ ${(produto.preco * produto.quantidade).toFixed(2)}`)
...
const converteTemperatura = temperaturas => temperaturas.map((temperatura) => `${temperatura} graus Celsius equivale a ${(temperatura * 9/5) + 32} graus na escala Fahrenheit`)
...
const somaNumeros = conjuntoNumeros => conjuntoNumeros.reduce((acumulador, valor) => acumulador + valor, 0)
...
const concatenaPalavras = conjuntoPalavras => conjuntoPalavras.reduce((acumulador, palavra) => acumulador + palavra)
...
const somaDespesas = despesasEmpresa => despesasEmpresa.map((despesa) => 
  `O total de despesas da categoria ${despesa.categoria} é de R$ ${despesa.valor.reduce((despesaAccumulator, valor) => despesaAccumulator + valor, 0)}`)
...
const encontraMaiorNumero = algunsNumeros => algunsNumeros.reduce((maior, numero) => 
  numero > maior ? numero : maior, algunsNumeros[0])
